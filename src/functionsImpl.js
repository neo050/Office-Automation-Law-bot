// ────────────────────────────────────────────────────────────────
//  src/functionsImpl.js   ·   Production build  (v2025‑08‑02)
//  Implements: client lookup, folder mgmt, media save, WhatsApp send,
//              GPT summary, Drive append‑update bundle.
//  Dependencies: axios, dayjs, openai, google‑drive client, logger.
// ────────────────────────────────────────────────────────────────
...
export async function saveMedia({ folderId, mediaId, mediaType }) {
  ...
  try {
    const url = await saveWhatsApp({ [mediaType]: { id: mediaId }, type: mediaType }, process.env.PERMANENT_WABA_TOKEN, folderId);
    return { ok: true, url };
  } catch (e) {
...
export async function saveChatBundleUpdate({ folderId, raw, summary }) {
  ...
  const upsert = async (name, body) => {
    // ① locate existing file in folder
    ...
    if (files.length) {
      ...
      const id       = files[0].id;
      const download = await drive.files.get({ fileId: id, alt: 'media' }, { responseType: 'stream' });
      const chunks   = [];
...
// Deprecated for new flow, but kept to avoid breaking old GPT prompt.
