...
async function sendWhatsAppSafe (args) {
  ...
  for (let i = 0; i < DELAYS.length; i++) {
    ...
    log.error('sendWhatsAppSafe', 'retry_attempt',
              { to: args.to, attempt: i, err: res.error });
...
async function setClientState (phone, state, ttl = REDIS_TTL_SEC) {
...
export async function agentHandle (waMsg) {
  ...
  if (!cState || Object.keys(cState).length === 0) {
    const sheet = await findClientByPhone(derivedPhone);
    if (sheet.ok && sheet.found) {
      cState = {
        phoneConfirmed   : true,
        fullNameConfirmed: !!sheet.fullName,
        fullName         : sheet.fullName || null
      };
      await setClientState(derivedPhone, cState, REDIS_TTL_SEC);
      log.debug('agentHandle', 'bootstrap_from_sheet',
                { phone: derivedPhone, cState });
    }
  }
  ...
  if (cState.awaitingPhone) {
    ...
    if (/^כן$/i.test(body)) {
      ...
      log.info('agentHandle', 'phone_overridden',
               { phone: derivedPhone, override: body });
    } else {
      await sendWhatsAppSafe({ to: derivedPhone,
                               text: 'לא זוהה מספר תקין. אנא השב/י "כן" או הקלד/י מספר מלא בפורמט 972…' });
      log.debug('agentHandle', 'bad_phone_format',
                { from: derivedPhone, body });
      return;
    }
  }
  ...
  if (!cState.fullNameConfirmed) {
    if (cState.awaitingFullName) {
      ...
      if (/^כן$/i.test(body) && cState.pendingName) {
        ...
        await setClientState(derivedPhone, cState);
        await sendWhatsAppSafe({ to: derivedPhone,
                                 text:`השם "${cState.fullName}" נשמר. נמשיך.` });
        log.info('agentHandle', 'fullName_confirmed',
                 { phone: derivedPhone, fullName:cState.fullName });
      } else if (NAME_RE.test(body)) {
        cState = { ...cState, awaitingFullName:true, pendingName:body };
        await setClientState(derivedPhone, cState, TMP_TTL_SEC);
        await sendWhatsAppSafe({
          to  : derivedPhone,
          text: `לאשר את השם "${body}"? השב/י "כן" או הקלד/י שם מתוקן.`
        });
        log.step('agentHandle', 'ask_fullName_confirm',
                 { phone: derivedPhone, candidate: body });
      } else {
        await sendWhatsAppSafe({ to: derivedPhone,
                                 text:'נא להקליד שם מלא (לפחות שתי מילים).' });
        log.debug('agentHandle', 'bad_fullName_format',
                  { phone: derivedPhone, body });
      }
      return;     // מחכים
    }

    await setClientState(derivedPhone,
                         { ...cState, awaitingFullName:true }, TMP_TTL_SEC);
    await sendWhatsAppSafe({ to: derivedPhone,
                             text:'נא להקליד את שמך המלא כפי שמופיע בת.ז.' });
    ...
    if (msg.tool_calls) {
      ...
      for (const tc of msg.tool_calls) {
        ...
        try {
          ...
          log.error('agentHandle',
                    `tool_${tc.function.name}_failed`, err);
  ...
  /* -------- Persist -------- */
  await redis.set(convKey, JSON.stringify(history), 'EX', REDIS_TTL_SEC);
